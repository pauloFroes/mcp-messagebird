...
export function registerTemplateTools(server: McpServer) {
    ...
    async ({ offset, limit, waba_id, channel_id }) => {
      try {
        const data = await apiRequest(
          "/v3/platforms/whatsapp/templates",
          "GET",
          undefined,
          {
            offset,
        ...
        return toolError(`Failed to list templates: ${(error as Error).message}`);
      }
    },
  );
  ...
  server.registerTool(
    ...
    async ({ name, language }) => {
      try {
        const data = await apiRequest(
          `/v3/platforms/whatsapp/templates/${name}/${language}`,
          "GET",
          undefined,
          undefined,
          "integrations",
        ...
        return toolError(`Failed to get template: ${(error as Error).message}`);
      }
    },
  );
  ...
  server.registerTool(
    ...
    async ({
      ...
      try {
        ...
        if (waba_id) requestBody.wabaId = waba_id;
        ...
        return toolError(`Failed to create template: ${(error as Error).message}`);
      }
    },
  );
  ...
  server.registerTool(
    ...
    async ({ name, language, waba_id, components, category }) => {
      try {
        ...
        const data = await apiRequest(
          `/v2/platforms/whatsapp/templates/${name}/${language}`,
          "PUT",
          body,
          undefined,
          "integrations",
        ...
        return toolError(`Failed to update template: ${(error as Error).message}`);
      }
    },
  );
  ...
  server.registerTool(
    ...
    async ({ name }) => {
      try {
        await apiRequest(
          `/v3/platforms/whatsapp/templates/${name}`,
          "DELETE",
          undefined,
          undefined,
          "integrations",
        ...
        return toolError(`Failed to delete template: ${(error as Error).message}`);
      }
    },
  );
  ...
  server.registerTool(
    ...
    async ({ name, language }) => {
      try {
        ...
        return toolError(`Failed to delete template variant: ${(error as Error).message}`);
      }
    },
  );
}

...
const CONVERSATION_STATUS = z
    ...
    .describe("Conversation lifecycle status (Conversations API only). " +
...
const PROXY_FILTER = z
    ...
    .describe("Client-side approximation (MessageBird Conversations API has no assignee field). " +
...
function isHumanAgentMessage(msg) {
    ...
    if (fullName === "Inbox Automation")
        return false;
    if (msg.source?.type === "ticketRule")
        return false;
    if (msg.origin === "inbox" && (agentId || inboxAgentId))
        return true;
    if (agentId || inboxAgentId)
        return true;
    return false;
}
function textPreview(msg, max = 160) {
 ...
 */
export function evaluateWaitingForReply(messages) {
    ...
    for (const msg of messages) {
        ...
        if (msg.direction === "received") {
            return {
                waiting: true,
                reason: "Last meaningful message is from the contact; no human Inbox agent replied after it.",
    ...
    return {
        waiting: false,
        reason: "No inbound contact message found in the recent window (or only automation traffic).",
    };
}
function agentFromSource(msg) {
    const src = msg.source || {};
    const agent = src.inboxAgent && (src.inboxAgent.id || "").trim()
        ? src.inboxAgent
 ...
 */
export function evaluateTicketProxy(messagesNewestFirst, conversationStatus) {
    ...
    for (const msg of chronological) {
        ...
        if (!et.startsWith("ticketLifecycle") && msg.type !== "event")
            continue;
        if (!et)
            continue;
        eventsSeen.push(et);
        ...
        else if (et === "ticketLifecycleUnassigned" ||
        ...
        else if (/resolved|closed|completed|done/i.test(et) ||
    ...
    const confidence = sawAssignOrUnassign
        ? "high"
        : "low";
    let note;
...
async function fetchMessages(conversationId, maxMessages) {
    const out = [];
    let offset = 0;
    const pageSize = 20;
    while (out.length < maxMessages) {
        const batch = Math.min(pageSize, maxMessages - out.length);
        const page = await apiRequest(`/conversations/${conversationId}/messages`, "GET", undefined, { limit: String(batch), offset: String(offset) });
...
export function registerConversationTools(server) {
    ...
    server.registerTool("list_conversations", {
        ...
        description: "List MessageBird Conversations API threads. " +
        ...
        inputSchema: {
            ...
            ids: z
                .string()
                .optional()
                .describe("Comma-separated conversation IDs to fetch (max 20). When set, other list filters still apply on the server side where supported."),
            offset: z.string().optional().describe("Number of items to skip (default: 0)"),
            limit: z
                .string()
                .optional()
                .describe("Max items to return (default: 20, max: 20). With a proxy filter, this is max matches."),
            proxy: PROXY_FILTER.optional(),
            waiting_for_reply: z
                .boolean()
                .optional()
                .describe("Deprecated alias: true ≡ proxy=waiting_for_reply. Prefer `proxy`."),
            scan_limit: z
                .string()
                .optional()
                .describe("With a proxy filter: max conversations to inspect (default: 40, max: 100). " +
                "Paginated in pages of 20."),
            message_scan_limit: z
                .string()
                .optional()
                .describe("With a proxy filter: max messages to load per conversation when reconstructing " +
    ...
    }, async ({ status, ids, offset, limit, proxy, waiting_for_reply, scan_limit, message_scan_limit, }) => {
        try {
            ...
            if (status && status !== "active" && status !== "all") {
                return toolError(`proxy=${mode} only applies to active conversations. ` +
            ...
            while (scanned < maxScan && matches.length < pageLimit) {
                ...
                const page = await apiRequest("/conversations", "GET", undefined, {
                    status: "active",
                    ids,
                ...
                for (const conv of items) {
                    ...
                    try {
                        ...
                        else if (mode === "assigned_open")
                            keep = ticket.assigned_open;
            ...
            return toolResult({
                ...
                filter: {
                    ...
                    ids: ids || null,
                    api: "MessageBird Conversations API only",
                    disclaimer: "assigned_open is reconstructed from ticketLifecycle* message events. " +
                    ...
                    definitions: {
                        waiting_for_reply: "Last non-event message is direction=received; no later human Inbox agent reply.",
                        assigned_open: "Conversation status=active AND latest ticketLifecycleAssigned " +
            ...
            return toolError(`Failed to list conversations: ${error.message}`);
        }
    });
    // --- Get conversation ---
    server.registerTool("get_conversation", {
        ...
        try {
            ...
            return toolError(`Failed to get conversation: ${error.message}`);
        }
    });
    // --- Update conversation ---
    server.registerTool("update_conversation", {
        ...
        try {
            const data = await apiRequest(`/conversations/${conversation_id}`, "PATCH", { status });
            return toolResult(data);
        }
        catch (error) {
            return toolError(`Failed to update conversation: ${error.message}`);
        }
    });
    // --- List messages in conversation ---
    server.registerTool("list_conversation_messages", {
        title: "List Conversation Messages",
        description: "List all messages in a specific conversation. Returns paginated message history " +
        ...
        try {
            const pageLimit = clampInt(limit, 20, 1, 20);
            const data = await apiRequest(`/conversations/${conversation_id}/messages`, "GET", undefined, { offset: offset ?? "0", limit: String(pageLimit) });
            return toolResult(data);
        }
        catch (error) {
            return toolError(`Failed to list messages: ${error.message}`);
        }
    });
    ...
    server.registerTool("get_message", {
        ...
        try {
            const data = await apiRequest(`/conversations/${conversation_id}/messages/${message_id}`);
            return toolResult(data);
        }
        catch (error) {
            return toolError(`Failed to get message: ${error.message}`);
        }
    });
    // --- Reply to conversation ---
    server.registerTool("reply_to_conversation", {
        title: "Reply to Conversation",
        description: "Send a reply message in an existing conversation. If the conversation is archived, a new one is created.",
        inputSchema: {
    ...
    }, async ({ conversation_id, type, text, media_url, caption, latitude, longitude, channel_id, }) => {
        try {
            let content;
            switch (type) {
                case "text":
            ...
            const data = await apiRequest(`/conversations/${conversation_id}/messages`, "POST", body);
            return toolResult(data);
        }
        catch (error) {
            return toolError(`Failed to reply to conversation: ${error.message}`);
        }
    });
    // --- List all messages ---
    server.registerTool("list_messages", {
        title: "List All Messages",
        description: "Fetch messages by IDs or by a short recent time window. " +
        ...
        inputSchema: {
            ...
            from: z
                .string()
                .optional()
                .describe("RFC3339 datetime lower bound (must be within the last 5 minutes). Required if `ids` is omitted."),
        },
        annotations: {
            readOnlyHint: true,
            destructiveHint: false,
        ...
        try {
            if (!ids && !from) {
                return toolError("list_messages requires either `ids` (comma-separated message IDs) " +

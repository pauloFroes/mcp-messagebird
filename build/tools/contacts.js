...
export function registerContactTools(server) {
    ...
    server.registerTool("list_contacts", {
        ...
        try {
            const data = await apiRequest("/contacts", "GET", undefined, { offset, limit }, "rest");
    ...
    server.registerTool("get_contact", {
        title: "Get Contact",
        description: "Get details of a specific contact by ID, phone number (msisdn), or name.",
        inputSchema: {
        ...
        try {
            if (contact_id) {
                const data = await apiRequest(`/contacts/${contact_id}`, "GET", undefined, undefined, "rest");
                return toolResult(data);
            }
            const params = {};
            if (msisdn)
            ...
            if (name)
                params.name = name;
            const data = await apiRequest("/contacts", "GET", undefined, params, "rest");
            return toolResult(data);
        }
        catch (error) {
            return toolError(`Failed to get contact: ${error.message}`);
        }
    });
    // --- Create contact ---
    server.registerTool("create_contact", {
        ...
        try {
            ...
            if (custom1)
                body.custom1 = custom1;
            if (custom2)
                body.custom2 = custom2;
            if (custom3)
                body.custom3 = custom3;
            if (custom4)
                body.custom4 = custom4;
            const data = await apiRequest("/contacts", "POST", body, undefined, "rest");
            return toolResult(data);
        }
        catch (error) {
            return toolError(`Failed to create contact: ${error.message}`);
        }
    });
    // --- Update contact ---
    server.registerTool("update_contact", {
        ...
        try {
            ...
            if (custom4)
                body.custom4 = custom4;
            const data = await apiRequest(`/contacts/${contact_id}`, "PATCH", body, undefined, "rest");
            return toolResult(data);
        }
        catch (error) {
            return toolError(`Failed to update contact: ${error.message}`);
        }
    });
    // --- Delete contact ---
    server.registerTool("delete_contact", {
        ...
        try {
            await apiRequest(`/contacts/${contact_id}`, "DELETE", undefined, undefined, "rest");
            return toolResult({ deleted: true, id: contact_id });
        }
        catch (error) {
            return toolError(`Failed to delete contact: ${error.message}`);
        }
    });
}

...
export function registerMessagingTools(server) {
    ...
    server.registerTool("send_text", {
        ...
        description: "Send a WhatsApp text message to a phone number. Use for simple text messages within or outside the 24h window (if template not required).",
        inputSchema: {
        ...
        try {
            ...
            if (report_url)
                body.reportUrl = report_url;
            const data = await apiRequest("/send", "POST", body);
            return toolResult(data);
        }
        catch (error) {
            return toolError(`Failed to send text message: ${error.message}`);
        }
    });
    // --- Send media message ---
    server.registerTool("send_media", {
        title: "Send Media Message",
        description: "Send a WhatsApp media message (image, video, audio, or file). Media URL must be publicly accessible.",
        inputSchema: {
        ...
        try {
            const mediaContent = { url: media_url };
            ...
            return toolError(`Failed to send media message: ${error.message}`);
        }
    });
    // --- Send location ---
    server.registerTool("send_location", {
        ...
        try {
            ...
            return toolError(`Failed to send location: ${error.message}`);
        }
    });
    // --- Send template (HSM) message ---
    server.registerTool("send_template", {
        title: "Send Template Message",
        description: "Send a WhatsApp template (HSM) message. Required to initiate conversations outside the 24h window. Template must be pre-approved by Meta.",
        inputSchema: {
            to: z.string().describe("Recipient phone number in E.164 format"),
            from: z.string().describe("WhatsApp Channel ID"),
    ...
    }, async ({ to, from, namespace, template_name, language_code, parameters, header_image_url, header_video_url, header_document_url, button_url_suffix, }) => {
        try {
            const hsm = {
                namespace,
            ...
            return toolError(`Failed to send template message: ${error.message}`);
        }
    });
    // --- Send interactive button message ---
    server.registerTool("send_interactive_buttons", {
        title: "Send Interactive Button Message",
        description: "Send a WhatsApp interactive message with quick reply buttons (max 3 buttons).",
        inputSchema: {
            to: z.string().describe("Recipient phone number in E.164 format"),
            from: z.string().describe("WhatsApp Channel ID"),
        ...
        try {
            ...
            return toolError(`Failed to send interactive buttons: ${error.message}`);
        }
    });
    // --- Send interactive list message ---
    server.registerTool("send_interactive_list", {
        title: "Send Interactive List Message",
        description: "Send a WhatsApp interactive list message with selectable menu options organized in sections.",
        inputSchema: {
            to: z.string().describe("Recipient phone number in E.164 format"),
            from: z.string().describe("WhatsApp Channel ID"),
            body_text: z.string().describe("Main message body text"),
        ...
        try {
            ...
            return toolError(`Failed to send interactive list: ${error.message}`);
        }
    });
}
